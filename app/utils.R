...
basemap <- function() {
  ...
  leaflet() %>%
    addProviderTiles(providers$CartoDB.DarkMatter, group = "Topographic") %>%
...
# map click --> get AOI climate raster
get_raster <- function(aoi, start = NULL, end = NULL, model) {
...
get_fires <- function(aoi, path) {
  fire <- sf::st_read(path) %>%
    ...
    sf::st_filter(st_transform(aoi, 4326)) %>%
    # st_union() %>%
...
fire_table <- function(df) {
  ...
  tbl <- rename(tbl,
                ...
                "Unit ID" = "Unit Id")
  # tbl$`Area (m²)` <- units::drop_units(tbl$`Area (m²)`)
  formattable(tbl,
              align = c("l", rep("r", NCOL(tbl) - 1)),
              list(`Fire name` = formatter("span", style = ~ style(color = "white",font.weight = "bold")),
                   `Acres burned` = color_tile2(c("yellow2", "darkorange2",  "firebrick3")),
                   # `Acres burned` = color_tile("yellow2",  "firebrick3"),
                   `Area` = formatter("span", style = ~ style(color = "white")),
                   `Year` = formatter("span", style = ~ style(color = "white",font.weight = "bold")),
                   `Unit ID` = formatter("span", style = ~ style(color = "white",font.weight = "bold")),
                   `Fire number` = formatter("span", style = ~ style(color = "white",font.weight = "bold"))))
...
# datatable(tbl, options = list(
#   initComplete = JS(
#     "function(settings, json) {",
#     "$('body').css({'font-family': 'Calibri'});",
#     "}"))) %>%
#   formatStyle("Acres burned",
#               backgroundColor  = styleInterval(c(0.25, 10, 100, 300, 1000, 5000, 99999999999),
#                                                c("grey,", 'lightyellow',
#                                                alpha('yellow', 0.6), alpha('gold', 0.9),
#                                                alpha('darkorange', 0.9), alpha("darkred", 0.8),
#                                                "darkred", "grey")))
